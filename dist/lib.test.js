...
describe("deg2rad", () => {
    it("should convert degrees to radians", () => {
        const degrees = 180;
        const radians = (0, lib_1.deg2rad)(degrees);
...
describe("calculateDistance", () => {
    it("should calculate the distance between two coordinates", () => {
        ...
        const distance = (0, lib_1.calculateDistance)(coordinateA, coordinateB);
...
describe("calculateBearing", () => {
    it("should calculate the bearing between two coordinates", () => {
        const coordinateA = { lat: 52.520008, lng: 13.404954 }; // Berlin, Germany
        const coordinateB = { lat: 45.421532, lng: -75.699642 }; // Ottawa, Canada
        const bearing = (0, lib_1.calculateBearing)(coordinateA, coordinateB);
...
describe("calculateBearingDiff", () => {
    it("should calculate the difference between two bearings less than 180 degrees", () => {
        const bearing1 = 45;
        const bearing2 = 90;
        const diff = (0, lib_1.calculateBearingDiff)(bearing1, bearing2);
        expect(diff).toBe(45);
    });
...
describe("filterPoints", () => {
    ...
    const points = Array.from({ length: 200 }, () => generateRandomCoordinates());
    it("filters out points that exceed bearing difference and distance threshold", () => {
        const threshold = 45;
        const filteredPoints = (0, lib_1.filterPoints)(start, end, points, threshold);
        filteredPoints.forEach((point) => {
            const bearing = (0, lib_1.calculateBearing)(start, point);
            const bearingDiff = (0, lib_1.calculateBearingDiff)(bearing, (0, lib_1.calculateBearing)(start, end));
            const distance = (0, lib_1.calculateDistance)(start, point);
            expect(bearingDiff).toBeLessThanOrEqual(threshold);
            expect(distance).toBeLessThanOrEqual((0, lib_1.calculateDistance)(start, end));
        });
    });
    ...
    it("returns an empty array if there are no points to filter", () => {
        const filteredPoints = (0, lib_1.filterPoints)(start, end, []);
...
describe("sortPoints", () => {
    ...
    it("should sort points based completely on distance if weighted as such", () => {
        const sortedPoints = (0, lib_1.sortPoints)(start, end, points, 1, 0);
        expect(sortedPoints).toEqual(points.sort((a, b) => (0, lib_1.calculateDistance)(start, a) - (0, lib_1.calculateDistance)(start, b)));
    });
    it("should sort points based completely on bearing if weighted as such", () => {
        const sortedPoints = (0, lib_1.sortPoints)(start, end, points, 0, 1);
        const bearing = (0, lib_1.calculateBearing)(start, end);
        expect(sortedPoints).toEqual(points.sort((a, b) => (0, lib_1.calculateBearingDiff)((0, lib_1.calculateBearing)(start, a), bearing) -
            (0, lib_1.calculateBearingDiff)((0, lib_1.calculateBearing)(start, b), bearing)));
    });
});
describe("rankPointsAlongBearing", () => {
    ...
    it("should rank points along the bearing line", () => {
        const rankedPoints = (0, lib_1.rankPointsAlongBearing)(start, end, points);
    ...
    it("should apply distance weight and bearing weight", () => {
        ...
        const rankedPoints = (0, lib_1.rankPointsAlongBearing)(start, end, points, distanceWeight, bearingWeight);
        expect(rankedPoints).toEqual([
            { lat: 1, lng: 1 },
            { lat: 2, lng: 2 },
            { lat: 3, lng: 3 },
    ...
    it("should respect the bearing difference threshold", () => {
        const threshold = 45;
        const rankedPoints = (0, lib_1.rankPointsAlongBearing)(start, end, points, lib_1.DISTANCE_WEIGHT, lib_1.BEARING_WEIGHT, threshold);
        expect(rankedPoints).toEqual([
            { lat: 1, lng: 1 },
            { lat: 2, lng: 2 },
            { lat: 3, lng: 3 },
